...
def show_main_menu(chat_id, user_id, message_id=None, edit=False):
    ...
    caption_text = (
        ...
        f"توکن PRS فرصتی برای ساخت آینده‌ای نوین روی بلاک‌چین است. سهم خودت را از این موج بزرگ بگیریم?\n\n"
...
def handle_all_messages(message):
    ...
    if text == "📊 وضعیت من و رتبه":
        ...
        guide_text = (
            f"📖 *راهنمای ولت و توکن PRS*\n\n"
            f"برای دریافت توکن‌ها کافی است یکی از کیف پول‌های معتبر مانند **تراست‌ولت (Trust Wallet)** یا **متاماسک (MetaMask)** را نصب کنید.\n\n"
            f"سپس با اد کردن (اضافه کردن) دستی توکن پرسپولیس (PRS) در ولت خود، آدرس ولت PRS را کپی کرده و در ربات ثبت نمایید.\n\n"
            f"💬 اگر هرگونه سوالی دارید، می‌توانید به پشتیبانی پیام دهید: @PRSsupportt"
        )
