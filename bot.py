...
def handle_all_messages(message):
    ...
    if text == "📊 وضعیت من و رتبه":
        ...
        guide_text = (
            ...
            f"🔹 **مقدمه:** برای دریافت توکن‌های PRS، به یک کیف پول معتبر ارز دیجیتال نیاز دارید که از شبکه‌های بلاک‌چین پشتیبانی کند. پیشنهاد ما استفاده از اپلیکیشن معتبر **Trust Wallet** است.\n\n"
            f"📱 **مرحله اول: نصب کیف پول**\n"
            f"• اپلیکیشن **Trust Wallet** را از گوگل‌پلی (برای اندروید) یا اپ‌استور (برای آیفون) دانلود و نصب کنید.\n"
            f"• وارد برنامه شده و روی گزینه **Create a new wallet** بزنید.\n"
            f"• کلمات بازیابی (Seed Phrase) ۱۲ کلمه‌ای را یادداشت کرده و در جای امنی نگه دارید (به هیچ عنوان به کسی ندهید).\n\n"
            f"🪙 **مرحله دوم: افزودن سفارشی توکن (در صورت نیاز)**\n"
            f"• در صفحه اصلی تراست ولت، روی علامت پلاس یا مدیریت توکن‌ها (Manage tokens) در بالا بزنید.\n"
            f"• شبکه را روی حالت صحیح قرار داده و آدرس قرارداد توکن پرسپولیس را وارد کنید تا توکن به لیست ولت شما اضافه شود.\n\n"
            f"📋 **مرحله سوم: کپی کردن آدرس ولت**\n"
            f"• در صفحه اصلی تراست ولت، ارز یا شبکه مورد نظر را پیدا کنید یا روی گزینه **Receive** بزنید.\n"
            f"• آدرس کیف پول خود (که با `0x` شروع می‌شود یا آدرس شبکه است) را کپی کنید.\n"
            f"• به ربات برگشته و از طریق دکمه «ارسال / ویرایش آدرس ولت»، آن را برای ما ارسال کنید تا در صف پرداخت قرار گیرید."
        )
