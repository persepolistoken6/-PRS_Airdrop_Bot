...
def handle_all_messages(message):
    ...
    if text == "📊 وضعیت من و رتبه":
        ...
        guide_text = (
            f"📖 *راهنمای کامل و گام‌به‌گام نصب کیف پول و ارسال آدرس (مخصوص افراد مبتدی):*\n\n"
            f"🔹 **مقدمه:** برای دریافت توکن‌های هواداری پرسپولیس (PRS)، شما به یک کیف پول امن ارز دیجیتال نیاز دارید. بهترین، امن‌ترین و ساده‌ترین پیشنهاد برای شما استفاده از اپلیکیشن معتبر **Trust Wallet (تراست ولت)** است.\n\n"
            f"📱 **مرحله اول: دانلود و نصب تراست ولت**\n"
            f"• اگر گوشی اندروید دارید، برنامه **Trust Wallet** را از گوگل‌پلی (Google Play) یا اگر آیفون دارید، از اپ‌استور (App Store) جستجو و دانلود کنید.\n"
            f"• برنامه را باز کرده و روی گزینه **Create a new wallet** (ساخت کیف پول جدید) بزنید.\n"
            f"• یک رمز عبور ۶ رقمی برای ورود به برنامه انتخاب کنید.\n\n"
            f"🔒 **مرحله بسیار مهم: یادداشت کلمات بازیابی (Seed Phrase)**\n"
            f"• در صفحه بعد، ۱۲ کلمه انگلیسی به شما نمایش داده می‌شود.\n"
            f"• **حتماً و حتماً** این ۱۲ کلمه را به ترتیب روی یک تکه کاغذ بنویسید و در جای بسیار امنی نگهداری کنید.\n"
            f"• ⚠️ به هیچ وجه این ۱۲ کلمه را به کسی ندهید؛ این کلمات حکم کلید گاوصندوق دیجیتال شما را دارند.\n"
            f"• در مرحله بعد، برنامه از شما می‌خواهد کلمات را به ترتیب انتخاب کنید تا مطمئن شود آن‌ها را یادداشت کرده‌اید.\n\n"
            f"🪙 **مرحله دوم: پیدا کردن و کپی کردن آدرس ولت**\n"
            f"• پس از ورود به صفحه اصلی تراست ولت، ارز یا شبکه مورد نظر (ترجیحاً شبکه اسمارت چین / BNB Smart Chain یا اتریوم که آدرس آن‌ها با `0x` شروع می‌شود) را پیدا کنید.\n"
            f"• روی آن ارز یا روی گزینه **Receive** (به معنای دریافت) بزنید.\n"
            f"• در صفحه باز شده، یک کپی از آدرس کیف پول خود (یک متن طولانی انگلیسی و اعداد که با `0x` شروع می‌شود) با زدن روی علامت کپی یا **Copy** بردارید.\n\n"
            f"📋 **مرحله سوم: ارسال به ربات**\n"
            f"• به همین ربات تلگرامی برگردید.\n"
            f"• از منوی اصلی روی دکمه **«ارسال / ویرایش آدرس ولت»** بزنید.\n"
            f"• آدرس کپی‌شده را در اینجا **Paste (چسباند)** کرده و ارسال کنید تا ثبت نهایی شما انجام شود."
        )
